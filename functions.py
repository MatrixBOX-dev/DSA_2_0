...
import varinit, createhtml, microcontroller, json, ipaddress, ssl
from varinit import *
import random
...
TXT = {
    ...
    "no_more_departures": "Keine weiteren Abfahrten",
}

# --- Ausfall-Laufzeile smooth ---
varinit.cancel_scroll_text = ""
varinit.cancel_scroll_ready = False
varinit.cancel_scroll_speed = 1
varinit.cancel_scroll_last = 0
varinit.cancel_scroll_delay = 0.025
varinit.cancel_scroll_width = 0
...
def strlen(_string): 
    return sum(
        varinit.fonts[varinit.currentfont][c][0]
        if c in varinit.fonts[varinit.currentfont]
        else varinit.fonts[varinit.currentfont]["_"][0]
        for c in _string
    )
...
def ping_screen():
    while varinit.ping:
        ...
        _data = fetch_data("data.t-skylt.se",443,"/get_departures?country=se&operator=sl&station=9001", )
...
def ad_message():
    ...
    data = fetch_data(host="data.t-skylt.se", port=443, args="/settings?ad")
    ad = json.loads(data)
...
def switch(_screen=True, _cls=False, force=False, wifi_screen=False):
    ...
    if _screen: 
        ...
        bounce(direction)
    cls(bottom, _refresh=False)
    reset_scroll()

def load_text():
    http = "" if int(varinit.settings["long"]) == -1 else "https://"
    if not varinit.settings["no_more_departures"]:
        varinit.settings["no_more_departures"] = "Keine weiteren Abfahrten"
...
def get_deviations():
    if not len(varinit.deviations_list):
        data = fetch_data(host="data.t-skylt.se", port=443, args="/v1/messages?future=false&transport_mode=METRO&transport_mode=TRAM&transport_mode=TRAIN&transport_mode=SHIP")
...
def convert_date(dt):
    ...
    try:
        varinit.today = dt.split(",")[0].replace(" ", "")
    except:
        pass

    try:
        # Beispiel: Thu, 07 May 2026 01:01:05 GMT
        ...
        print("Date parse error:", e, dt)
        return varinit.currenttime

    if not varinit.first_start:
        utc_offset = int(varinit.settings.get("utc_offset", 1))
        summer_time = int(varinit.settings.get("summer_time", 0))
        
        local_hour = (int(_hour) + utc_offset + summer_time) % 24
        varinit._currenttime = "{:02d}:{}".format(local_hour, _minute)

    _newtime = time.mktime(time.struct_time((
        int(_year),
        int(_month),
        int(_date),
        int(_hour),
...
def disp_init():
    ...
    varinit.tg2.x = 0; varinit.tg2.y = 16
    varinit.tg3.y = 32
...
def draw_text_to_bitmap(bitmap, text, x_offset=0):
    font = fonts[1]
    pixwidth = x_offset
    ...
    for character in text:
        ...
        for width in range(font[character][0]):
            for height in range(font["fontheight"]):
                ...
                if color:
                    try:
                        bitmap[pixwidth + width, height] = 1
                    except:
                        pass
...
def setup_cancel_scroll():
    if not varinit.cancel_scroll_text:
        return

    font_before = varinit.currentfont
    varinit.currentfont = 1
    text_width = strlen(varinit.cancel_scroll_text)
    varinit.currentfont = font_before

    scroll_width = text_width + varinit.if_long + 20

    scroll_bitmap = displayio.Bitmap(scroll_width, 8, 2)

    scroll_palette = displayio.Palette(2)
    scroll_palette[0] = 0x000000
    scroll_palette[1] = varinit.palette[1]
    scroll_palette.make_transparent(0)
    ...
    try:
        varinit.group.remove(varinit.cancel_scroll_tg)
    except:
        pass

    varinit.cancel_scroll_tg = displayio.TileGrid(
        scroll_bitmap,
        pixel_shader=scroll_palette,
        x=0,
        y=24
    )

    varinit.group.append(varinit.cancel_scroll_tg)

    # schwarze Maske rechts
    mask_width = 42

    mask_bitmap = displayio.Bitmap(mask_width, 8, 2)
    ...
    try:
        varinit.group.remove(varinit.cancel_mask_tg)
    except:
        pass

    varinit.cancel_mask_tg = displayio.TileGrid(
        mask_bitmap,
        pixel_shader=mask_palette,
        x=varinit.if_long - mask_width,
        y=24
    )

    varinit.group.append(varinit.cancel_mask_tg)
    # Uhr als oberstes Layer
    clock_bitmap = displayio.Bitmap(40, 8, 2)
    ...
    try:
        varinit.group.remove(varinit.cancel_clock_tg)
    except:
        pass

    varinit.cancel_clock_tg = displayio.TileGrid(
        clock_bitmap,
        pixel_shader=clock_palette,
        x=varinit.if_long - 35,
        y=24
    )

    varinit.group.append(varinit.cancel_clock_tg)
    varinit.cancel_clock_last = ""
    varinit.cancel_scroll_width = text_width
    varinit.cancel_scroll_ready = True


def tick_cancel_scroll():
    if not varinit.cancel_scroll_text:
        return

    if not varinit.cancel_scroll_ready:
        setup_cancel_scroll()

    if time.monotonic() < varinit.cancel_scroll_last + varinit.cancel_scroll_delay:
        return

    varinit.cancel_scroll_last = time.monotonic()

    try:
        varinit.cancel_scroll_tg.x -= varinit.cancel_scroll_speed

        if varinit.cancel_scroll_tg.x < -(varinit.cancel_scroll_width + varinit.if_long):
            varinit.cancel_scroll_tg.x = 0
    except:
        varinit.cancel_scroll_ready = False

def draw_cancel_clock():
    if not varinit.cancel_scroll_ready:
        return

    current_clock = "°" + varinit._currenttime

    if getattr(varinit, "cancel_clock_last", "") == current_clock:
        return

    varinit.cancel_clock_last = current_clock

    for x in range(40):
        for y in range(8):
            varinit.cancel_clock_tg.bitmap[x, y] = 0

    draw_text_to_bitmap(
        varinit.cancel_clock_tg.bitmap,
        current_clock,
        0
    )
...
def sort_by_hours(lst):
    def to_minutes(row):
        ...
        now_h = int(varinit._currenttime[:2])
        now_m = int(varinit._currenttime[3:5])
        now = now_h * 60 + now_m
    ...
    return sorted(lst, key=to_minutes)
...
def get_api_request(num):
    station_id = varinit.settings["stations"][num].get("siteid", "de:14522:70048")

    api_provider = int(varinit.settings.get("api_provider", 1))
    ...
    if api_provider == 1:
        return (
            "www.bahn.de",
            443,
            "/web/api/reiseloesung/abfahrten?ortExtId="
            + str(station_id)
            + "&verkehrsmittel[]=ICE&verkehrsmittel[]=EC_IC&verkehrsmittel[]=IR&verkehrsmittel[]=REGIONAL&verkehrsmittel[]=SBAHN&mitVias=true&maxVias=2"
        )
    ...
    if api_provider == 3:
        return (
            "efa.vvo-online.de",
            443,
            "/VMSSL3/XSLT_DM_REQUEST?language=de&stateless=1&type_dm=stop&name_dm="
            + str(station_id)
            + "&mode=direct&useRealtime=1&limit=10&outputFormat=json"
        )
    ...
    try:
        ...
        if api_provider == 3:
            ...
            if isinstance(data, list):
                ...
                deps = data.get("entries", [])
        else:
            deps = []

        print("departures:", len(deps) if deps else 0)
    
        if not deps:
            varinit.cancel_scroll_text = ""
            varinit.cancel_scroll_ready = False
        
            for tg in ("cancel_scroll_tg", "cancel_mask_tg", "cancel_clock_tg"):
                try:
                    varinit.group.remove(getattr(varinit, tg))
                except:
                    pass
        ...
        for dep in deps[:10]:
            ...
            if api_provider == 3:
                ...
                dt = dep.get("dateTime", {}) or {}
                hour = dt.get("hour", None)
                minute = dt.get("minute", None)

                try:
                    delay = int(serving.get("delay", None))
                    if delay == -9999:
                        cancelled = True
                        delay = None
                except:
                ...
                platform_text = dep.get("platformName", "")
                if not platform_text:
                    platform_text = dep.get("platform", "")
                
                attrs = dep.get("attrs", [])
                platform_changed = any(
                    attr.get("name") == "platformChange"
                    and attr.get("value") == "changed"
                    for attr in attrs
                )
                
                if platform_changed and platform_text:
                ...
                if not dest:
                    ...
                    if isinstance(ueber, dict):
                        dest = str(ueber.get("1") or "").strip()
                
                    elif isinstance(ueber, list) and len(ueber) > 1:
                        dest = str(ueber[1] or "").strip()

                for key in ("zeit", "ezZeit", "time", "plannedTime", "abfahrtsZeitpunkt", "departureTime"):
                    t = str(dep.get(key, ""))

                    if "T" in t and ":" in t:
                        hour = t.split("T")[1][0:2]
                        minute = t.split("T")[1][3:5]
                        break
                ...
                meldungen = dep.get("meldungen", [])

                for msg in meldungen:
                    typ = str(msg.get("type", ""))

                    if typ == "DEPARTURE_CANCELLED" or typ == "TRIP_CANCELLED" or typ == "HALT_AUSFALL":
                        cancelled = True
                        break

                planzeit = dep.get("zeit", "")
                echtzeit = dep.get("ezZeit", "")

                if planzeit and echtzeit:
                    try:
                        plan_min = time_to_minutes(planzeit)
                        echt_min = time_to_minutes(echtzeit)
                
                        delay = echt_min - plan_min
                ...
                if dep.get("ezGleis"):
                    platform_text = "~" + dep["ezGleis"]
                else:
                    platform_text = dep.get("gleis", "")

            if hour is None or minute is None:
                return [["1", "Zeitfeld fehlt", "", "", ""]]
            ...
            scroll_time_text = str(hour).replace("|", "") + ":" + str(minute).replace("|", "")
            ...
            if cancelled:
                ...
                if cancel_msg not in new_cancel_scroll_text:
                    new_cancel_scroll_text += cancel_msg
            ...
            if not cancelled:
                try:
                    ...
                    for attr in attrs:
                        if attr.get("name") == "EarlyTermination":
                            early_termination = attr.get("value")
                            ...
                            reroute_msg = (
                                ...
                                + scroll_time_text
                                + " Uhr, heute nur bis "
                                + str(early_termination)
                                + ".     ***     "
                            )

                            if reroute_msg not in new_cancel_scroll_text:
                                new_cancel_scroll_text += reroute_msg
            ...
            dataout.append(["0", line, dest, time_text, delay_text, platform_text])
    
        if new_cancel_scroll_text != varinit.cancel_scroll_text:
            varinit.cancel_scroll_text = new_cancel_scroll_text
            varinit.cancel_scroll_ready = False
...
def reformat_data(trainlist):
    def draw_departure(row, screen_partition):
        layout = int(varinit.settings.get("layout", 1))
        try:
            row[1] = row[1][:varinit.settings["line_length"]]
        except:
            pass
        ...
        if varinit.if_long == 128:
            if row[2] in station_names_dict:
                row[2] = station_names_dict[row[2]]

        time_text = row[3]
        line_text = str(row[1]).upper()
        dest_text = row[2]
        platform_text = row[5] if len(row) > 5 else ""
        # wird jetzt außerhalb einmal komplett gelöscht
        pass
        ...
        time_length = strlen(time_text) + 2

        font_before = varinit.currentfont
        
        varinit.currentfont = varinit.custom_font_index
        line_width = strlen(line_text)
        x_dest = 48 + line_width + (2 if line_width > 0 else 0)
        
        varinit.currentfont = 0
        platform_width = strlen(platform_text)
        
        varinit.currentfont = font_before
        
        x_line_right = x_dest - min_gap - line_width
        x_platform_right = varinit.if_long - platform_width

        row_y = 0 if screen_partition == 1 else 12

        font_before = varinit.currentfont
        varinit.currentfont = varinit.custom_font_index
        
        if layout == 1:
            renderstring(line_text, 0, x_offset=50, absolute_y=row_y + 3)        
            varinit.currentfont = font_before
            renderstring(row[4], 0, smallfont=True, x_offset=30, absolute_y=row_y + 2)

        renderstring(time_text, 0, large=True, x_offset=-1, absolute_y=row_y)

        if layout == 1:
            renderstring(dest_text, 0, large=True, x_offset=x_dest + 1, absolute_y=row_y)

        if layout == 2:
            renderstring(dest_text, 0, smallfont=True, x_offset=time_length, absolute_y=row_y + 2)

        renderstring(platform_text, 0, large=True, x_offset=x_platform_right, absolute_y=row_y)

        if screen_partition == 0:
            current_clock = "°" + varinit._currenttime

            clock_x = varinit.if_long - 35
    ...
    def draw_message(msg):
        renderstring(msg, 0, large=True, absolute_y=10)
        return ""

    def draw_centered_message(msg=None):
        cls(topbottom)

        line1 = "Bitte beachten Sie"
        line2 = "den Fahrplanaushang!"

        x1 = (varinit.if_long - strlen(line1)) // 2
        x2 = (varinit.if_long - strlen(line2)) // 2

        renderstring(line1, 0, large=True, x_offset=x1, absolute_y=0)
        renderstring(line2, 0, large=True, x_offset=x2, absolute_y=12)

        current_clock = "°" + varinit._currenttime
        clock_x = varinit.if_long - 35

        renderstring(current_clock, 0, smallfont=True, x_offset=clock_x, absolute_y=24)
    ...
    elif int(varinit.settings["listmode"]):
        return trainlist

    else:
        cls(topbottom)

        draw_departure(trainlist[0], 1)

        if len(trainlist) > 1:
            draw_departure(trainlist[1], 0)
        else:
            current_clock = "°" + varinit._currenttime
            clock_x = varinit.if_long - 35
...
def renderstring(_string, screen_partition = 0, min = 0, slow = 0, invertcolor = 0, shading=False, smallfont=False, sys_msg=False, shade=False, large=False, _cls=False, _refresh=False, ontop=False, block=False, logo=False, mini=False, x_offset=0, target_halves=False, y_offset=0, absolute_y=None):
    ...
    if TXT["no_more_departures"] in _string:
        varinit.shared["nightcount"] += 1
    ...
    if absolute_y is not None:
        screen_location = [topbottom, topbottom, topbottom, topbottom]
        screen_partition = 0
        offs = absolute_y
    elif target_halves:
        screen_location = [bottom, top, top, bottom]
    elif varinit.currentfont:
        screen_location = [topbottom, topbottom, topbottom, topbottom]
    else:
        screen_location = [bottom, top, top, bottom]
...
def scroll_mode():
    ...
    nightcheck()
    varinit.tg1.y, varinit.tg2.y, varinit.tg3.y = -32, -32, 0
    ...
    if varinit.shared["loop_counter"] == -7: reset()
        ...
        if varinit.new_version_available == 1:
            scroll_buff += " -    ^ " + TXT["new_version_available"] + str(varinit.new_ver)
        ...
        varinit.text[5] = direction + "   >   " + str(varinit.settings["maxdest"]) + TXT["departures"] + ifoffset
    ...
    if varinit.shared["loop_counter"] > 0:
        if varinit.active_message == True:
            ...
            if False:
                ...
                varinit.scrollsum = renderstring(reformat_data(get_departure()), large=True)
    ...
    try:
        ...
        for record in varinit.traindata:
            ...
            if not half and "str" in str(type(trainlist)): 
                ...
                if TXT["no_more_departures"] in trainlist:
...
def list_splash(_settings=False):
    ...
    if varinit.shared["startup"] == 1 or _settings:
        ...
        if int(varinit.settings["stations"]["1"]["offset"]):
            sysprint(TXT["hiding"] + str(varinit.settings["stations"]["1"]["offset"]) + varinit.settings["mins"], 102, _refresh=False)
...
def update_screen():
    ...
    renderstring("1. " + TXT["connect_to"], 100, shading=True, smallfont=True, _cls=topbottom)
    ...
    renderstring("2. " + TXT["go_to"], lastrow, shading=True, smallfont=True)
...
def savesettings(_settings=varinit.settings, saved=TXT["saving"]):
